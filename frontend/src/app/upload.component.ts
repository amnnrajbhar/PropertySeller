          ...
          <div *ngFor="let file of uploadedFiles" class="media-card" (click)="openModal(file)">
            ...
            <div class="media-info">
              ...
              <div class="action-icons">
                <button class="icon-btn" (click)="shareWhatsApp(file, $event)" title="Share on WhatsApp">
                  <span class="material-icons">whatsapp</span>
                </button>
                <button class="icon-btn" (click)="sharePhone(file, $event)" title="Share via Phone">
                  <span class="material-icons">phone</span>
                </button>
                <button class="icon-btn" (click)="shareFile(file, $event)" title="Share">
                  <span class="material-icons">share</span>
                </button>
                <button class="icon-btn delete" (click)="deleteFile(file, $event)" title="Delete">
        ...
        <div class="modal-content" (click)="$event.stopPropagation()">
          ...
          <div class="modal-info">
            ...
            <div class="modal-actions">
              <button class="action-btn" (click)="shareWhatsApp(selectedMedia, $event)">
                <span class="material-icons">whatsapp</span> WhatsApp
              </button>
              <button class="action-btn" (click)="sharePhone(selectedMedia, $event)">
                <span class="material-icons">phone</span> Phone
              </button>
              <button class="action-btn" (click)="shareFile(selectedMedia, $event)">
                <span class="material-icons">share</span> Share
              </button>
              <a [href]="selectedMedia.url" target="_blank" class="action-btn download">
                <span class="material-icons">download</span> Download
              </a>
              <button class="action-btn delete" (click)="deleteFileFromModal()">
...
export class UploadComponent implements OnInit {
  ...
  shareWhatsApp(file: S3File, event: Event): void {
    event.stopPropagation();
    const text = `Check out this file: ${file.fileName}`;
    const url = `https://wa.me/?text=${encodeURIComponent(text + ' ' + file.url)}`;
    window.open(url, '_blank');
  }

  sharePhone(file: S3File, event: Event): void {
    event.stopPropagation();
    const text = `Check out this file: ${file.fileName}\n${file.url}`;
  ...
  shareFile(file: S3File, event: Event): void {
    event.stopPropagation();
    if (navigator.share) {
      navigator.share({
        title: file.fileName,
